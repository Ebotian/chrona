...
const isDetailOpen = computed(() => route.name === 'article-detail' && Boolean(activeSlug.value))
...
async function onTagClick(payload: TagClickPayload) {
  ...
  if (monthRect) monthPickerCenterYGlobal.value = monthRect.top + monthRect.height / 2
  ...
  if (rowContainer && clickedRow) {
    const allRows = Array.from(rowContainer.querySelectorAll('.row-band')) as HTMLElement[]
...
async function animateRowBackToOrigin(): Promise<void> {
  ...
  const targetTop = frozenRowStartTopGlobal.value || (stageRect.top + stageRect.height / 2 - rowHeight / 2)
  const localTargetTop = targetTop - stageRect.top
  ...
  <div class="home-container">
    ...
    <main class="center">
      <section class="timeline-col" :class="{ locked: isDetailOpen }">
        <Timeline :key="`timeline-${isDetailOpen ? 'detail' : 'list'}`" :external-jump-token="monthJumpToken" />
      </section>
      <section class="tag-col">
        <div ref="monthPickerSlotRef" class="month-picker-slot" :class="{
        ...
        <section ref="sharedStageRef" class="shared-stage">
          ...
          <Transition name="shared-swap" mode="out-in">
            <TagRegion v-if="!isDetailOpen" key="tag-region" :entries="timelineEntries" @tag-click="onTagClick" />
            <div v-else :key="`detail-${activeSlug}`" class="detail-stage" :class="{ closing: isClosingDetail }">
              <button class="detail-exit" type="button"
                :style="{ top: `${frozenRowTopLocal + frozenRowHeight + DETAIL_GAP}px` }"
                @click="onDetailClose">返回</button>
              <div class="detail-content-host" :style="{
                top: `${frozenRowTopLocal + frozenRowHeight + DETAIL_GAP + EXIT_BTN_HEIGHT + DETAIL_GAP}px`
              }">
                <DetailPage :slug="activeSlug || ''" embedded :show-header="false" :show-title="false"
                  :content-offset="0" />
              </div>
            </div>
          </Transition>
...
.month-picker-slot {
  opacity: 1;
  transition: opacity 10ms linear, flex-basis 180ms ease, max-height 180ms ease;
  max-height: 3.375rem;
  overflow: hidden;
}
...
.detail-stage.closing .detail-exit,
